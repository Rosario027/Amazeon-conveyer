...
export const api = {
  ...
  deleteProject: (id, reason) => request(`/api/projects/${id}`, { method: 'DELETE', body: JSON.stringify({ reason }) }),
  restoreProject: (id) => request(`/api/projects/${id}/restore`, { method: 'POST', body: JSON.stringify({}) }),
  setProjectStage: (id, stage, note) => request(`/api/projects/${id}/stage`, { method: 'POST', body: JSON.stringify({ stage, note }) }),
  projectStages: () => request('/api/projects/stages'),
  downloadProjects: () => download('/api/projects/export.xlsx', 'Projects.xlsx'),
  // partners (profit share + withdrawals)
  partnerOverview: () => request('/api/partners/overview'),
  addWithdrawal: (data) => request('/api/partners/withdrawals', { method: 'POST', body: JSON.stringify(data) }),
  updateWithdrawal: (id, data) => request(`/api/partners/withdrawals/${id}`, { method: 'PUT', body: JSON.stringify(data) }),
  deleteWithdrawal: (id) => request(`/api/partners/withdrawals/${id}`, { method: 'DELETE' }),
  downloadPartnerReport: () => download('/api/partners/overview.xlsx', 'Partner-Profit-Share.xlsx'),
  downloadMovements: (from, to) => download(`/api/partners/movements.xlsx${qs({ from, to })}`, 'Drawings-and-Capital.xlsx'),
  settingsLog: (limit) => request(`/api/settings/log${qs({ limit })}`),
