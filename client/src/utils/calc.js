// Client mirror of server/lib/calc.js — keeps the live preview identical
// to what the server stores and prints.
...
export function computeTotals(invoice) {
  ...
  const lineItems = items.map((it, idx) => {
    ...
    return { ...it, slNo: idx + 1, qty, rate, gstRate, taxable: round2(qty * rate) };
  });
  ...
  return {
    ...
    cgstAmount, sgstAmount, igstAmount, totalTax, roundOff, grandTotal,
